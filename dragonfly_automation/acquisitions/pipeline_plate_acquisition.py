...
class PipelinePlateAcquisition:
    ...
    def select_positions(self, positions, last_afc_updated_focusdrive_position=None):
        ...
        afc_did_succeed = self.operations.call_afc(
            self.micromanager_interface, self.event_logger, self.afc_logger, positions[0]['ind']
        )
        ...
        afc_updated_focusdrive_position = self.micromanager_interface.mm_core.getPosition(
            'FocusDrive'
        )
        ...
        if num_positions_short > 0:
            ...
            if len(additional_positions):
                ...
                self.event_logger(
                    'ACQUISITION INFO: Fewer than the minimum of %s acceptable FOVs were found '
                    'and there are no additional scored FOVs to image' % (min_num_positions,)
                )
    ...
    def acquire_positions(self, positions):
        ...
        if not afc_did_succeed:
            self.event_logger(
                ...
                'so autoexposure cannot be run and stacks cannot be acquired' % self.current_well_id
            )
            return
