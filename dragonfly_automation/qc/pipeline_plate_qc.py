...
class PipelinePlateQC:
    ...
    def parse_score_log(self):
        ...
        summary = {
            ...
            'pct_scores_gt_phalf': int(100 * (self.score_log.score > 0.5).sum() / num_fovs),
            'pct_scores_le_nhalf': int(100 * (self.score_log.score < -0.5).sum() / num_fovs),
    ...
    def validate_external_metadata(self, external_metadata):
        ...
        if md['platemap_type'] not in ['first-half', 'second-half', 'custom', 'none']:
            raise ValueError(
                "Invalid platemap_type '%s' in %s" % (md['platemap_type'], self.root_dirname)
            )
    ...
    def load_and_validate_custom_platemap(self):
        ...
        required_columns = set(EXTERNAL_METADATA_SCHEMA['properties'].keys()).difference(
            ['pml_id', 'platemap_type']
        )
    ...
    def load_platemap(self):
        ...
        if platemap_type == 'first-half':
            ...
            plate_layout = [
                {'imaging_well_id': well_id, 'pipeline_well_id': well_id} for well_id in well_ids
            ]
    ...
    def tile_acquired_fovs(self, channel_ind=0, save_plot=False):
        ...
        horizontal_border = (255 * np.ones((border_width, 2 * im_sz + border_width))).astype(
            'uint8'
        )
        ...
        for row_ind, row_label in enumerate(rows):
            for col_ind, col_label in enumerate(cols):
                ...
                if well_aq_log.shape[0]:
                    ...
                    gfp_laser_power = (
                        '%d' % gfp_laser_power if gfp_laser_power > 1 else '%0.2f' % gfp_laser_power
                    )
        ...
        plt.subplots_adjust(left=0.01, right=0.99, top=0.95, bottom=0.01, wspace=0.01, hspace=0.15)
        plt.savefig(
            os.path.join(
                self.qc_dir, '%s-top-scoring-FOVs-CH%d.pdf' % (self.root_dirname, channel_ind)
            )
        )
